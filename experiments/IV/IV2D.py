...
for vin1 in np.linspace(V_low,V_high,50):
	for vin2 in np.linspace(Vg_low,Vg_high,50):
		x=np.ones([2,siglen*Fs])
		t=np.arange(len(x[0,:]))*1/Fs
		x[0,:]=x[0,:]*vin1+np.random.normal(0, 1, size=len(x[0,:]))*0.01+0.001*np.sin(2*np.pi*23*t)
		x[1,:]=x[1,:]*vin2
