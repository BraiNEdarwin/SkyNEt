...
outputs = np.zeros((cf.sampleTime.shape[0], cf.n, int(cf.sampleTime[-1]*cf.fs))) # length of outputs is defined by longest measurement
...
print('Starting gradient convergence for increasing sample times experiment. Estimated duration: ' + str(np.sum(cf.n*cf.sampleTime)/60 + cf.n*cf.sampleTime.shape[0]*(0.2 + 2*cf.rampT)/60) + ' minutes.')
for g in range(cf.sampleTime.shape[0]):
    print('Sampling for sample time ' + str(g+1) + '/' + str(cf.sampleTime.shape[0]) + ' (' + str(cf.sampleTime[g]) +'s)')
    ...
    for i in range(cf.n):       
        ...
        if cf.device == 'chip':
            ...
            outputs[g,i,0:int(cf.sampleTime[g]*cf.fs)] = dataRamped[0, int(cf.fs*cf.rampT):-int(cf.fs*cf.rampT)]   # Cut off the ramping up and down part 
        elif cf.device == 'NN':
            outputs[g,i,0:int(cf.sampleTime[g]*cf.fs)] = net.outputs(torch.from_numpy(inputs[:,:int(cf.sampleTime[g]*cf.fs)].T).to(torch.float)) + np.random.normal(0,0.5,data[i,:].shape[0])     
            
        # Lock-in technique to determine gradients        
        IVgrad[g,i,:], phases[g,i,:] = cf.lock_in_gradient(outputs[g,i,0:int(cf.sampleTime[g]*cf.fs)], cf.freq, cf.waveAmplitude, cf.fs, cf.phase_thres)
    ...
    if cf.device == 'chip':
        ...
        SaveLib.saveExperiment(cf.configSrc, saveDirectory,
                                 controls = controls,
                                 staticControls = cf.staticControls,
                                 inputs = inputs,
                                 output = outputs,
                                 freq = cf.freq,
