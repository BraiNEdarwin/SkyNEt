...
#TODO: add cf.labels to automatically search for different labels
if cf.initializations > 1: 
...
for exps in range(cf.initializations):
  ...
  for i in range(cf.n + 1):  
      ...
      if i != cf.n:
          if i > 0 and error[i-1] < 0.4:
            stop = True # Measure one last time without sine waves
            break
      ...
      if cf.verbose:
        ...
        PlotBuilder.fitnessMainEvolution(mainFig,
                                               ...
                                               i + 1)
      if stop == True:
        print('Stopping criterium met, stopping experiment.')
        break
