## code gejat van IV_switch en switch to device
...
import numpy as np
import os
import config_IV_choose_device as config
import time
...
grounded_input = np.zeros((2, Input.shape[0]))
grounded_input[0] = Input

# Initialize serial object
ser = InstrumentImporter.switch_utils.init_serial(config.comport)

# Switch to device
InstrumentImporter.switch_utils.connect_single_device(ser, config.device)
