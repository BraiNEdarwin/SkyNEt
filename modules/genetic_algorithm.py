...
# ------------------------------------------------------------------------------
    ...
    def crossover_blxab(self, parent1, parent2):
        ...
        maxmin_diff = (maximum - minimum)
        offspring = np.zeros((parent1.shape))
        for i in range(len(parent1)):
            if parent1[i] > parent2[i]:
                offspring[i] = np.random.uniform(minimum[i] - maxmin_diff[i] * beta, maximum[i] + maxmin_diff[i] * alpha)
            else:
                offspring[i] = np.random.uniform(minimum[i] - maxmin_diff[i] * alpha, maximum[i] + maxmin_diff[i] * beta)
        for i in range(0, self.genes):
