...
# %% Hardware chip platform to measure the current output from voltage configurations of disordered NE systems
class Hardware:
    def __init__(self, platform_dict):
        pass

    def evaluate_population(self, inputs_wfm, gene_pool, target_wfm):
        # TODO: Implement hardware chip platform
        print('This platform is still not implemented')
        pass


# %% Platform based on software simulation of the chip with Neural Networks using models loaded form staNNet
class SimulationNN:

    def __init__(self, platform_dict):
        # Import required packages
        self.accelerator = importlib.import_module('SkyNEt.config.acceleration').Accelerator
    ...
    def evaluate_population(self, inputs_wfm, gene_pool, target_wfm):
        ...
        for j in range(genomes):
            ...
            x_trafo = self.trafo(inputs_wfm, gene_pool[j, -1])

            # Feed input to NN
            # target_wfm.shape, genePool.shape --> (time-steps,) , (nr-genomes,nr-genes)
            g = np.ones_like(target_wfm)[:, np.newaxis] * gene_pool[j, :5, np.newaxis].T
            ...
            output = self.net.outputs(self.accelerator.format_numpy(x_dummy))
...
class SimulationKMC:
    ...
    def evaluate_population(self, inputs_wfm, gene_pool, target_wfm):
        # TODO: Implement kinetic Monte Carlo simulation platform
        print('This platform is still not implemented')
        pass
