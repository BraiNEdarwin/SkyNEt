...
def reset_device():
	dev = device.Device(name='cDAQ1Mod1')
	dev.reset_device()
...
def IO(y, Fs, inputPorts = [1, 0, 0, 0, 0, 0, 0]):
    ...
    with nidaqmx.Task() as output_task, nidaqmx.Task() as input_task:
      # Define ao/ai channels
        for i in range(n_ao):
            output_task.ao_channels.add_ao_voltage_chan('cDAQ1Mod1/ao'+str(i+6)+'', 'ao'+str(i)+'', -5, 5)
        for i in range(len(inputPorts)):
            if(inputPorts[i] == 1):
                input_task.ai_channels.add_ai_voltage_chan('cDAQ1Mod2/ai'+str(i+0)+'') 
